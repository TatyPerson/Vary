 ...
 * in SonarQube
 */
public abstract class VaryReportSensor implements Sensor {
   ...
   * @param reactor
   */
  protected VaryReportSensor(Settings conf, FileSystem fs, ProjectReactor reactor) {
   ...
   * @param reactor
   */
  protected VaryReportSensor(ResourcePerspectives perspectives, Settings conf, FileSystem fs, ProjectReactor reactor, Metric metric) {
    this.conf = conf;
    this.fs = fs;
    this.metric = metric;
  ...
  public boolean shouldExecuteOnProject(Project project) {
    return fs.hasFiles(fs.predicates().hasLanguage(VaryLanguage.KEY));
  }
  ...
  public void analyse(Project project, SensorContext context) {
    if (!VaryUtils.isReactorProject(project)) {
      try {
      List<File> reports = getReports(conf, reactor.getRoot().getBaseDir().getCanonicalPath(),
        ...
        try {
          ...
          VaryUtils.LOG.info("{} processed = {}", metric == null ? "Issues" : metric.getName(),
  ...
  private boolean saveViolation(Project project, SensorContext context, String ruleRepoKey,
    ...
    if ((filename != null) && (filename.length() > 0)) {
      ...
      String normalPath = VaryUtils.normalizePathFull(filename, root);
      if (normalPath != null && !notFoundFiles.contains(normalPath)) {
        ...
        if (context.getResource(file) != null) {
          ...
          VaryUtils.LOG.warn("Cannot find the file '{}', skipping violations", normalPath);
  ...
  private boolean contextSaveViolation(Resource resource, int lineNr, RuleKey rule, String msg) {
    ...
    if (issuable != null) {
      ...
      try{
        ...
        VaryUtils.LOG.error("Could not add the issue, details: '{}'", me.toString());
      }
      if (result)
        violationsCount++;
    }
  ...
  private int getLineAsInt(String line) {
    ...
    if (line != null) {
      try {
        ...
        VaryUtils.LOG.warn("Skipping invalid line number: {}", line);
        lineNr = -1;
      }
    }
    return lineNr;
