package org.sonar.vary.plugin.highlighter;
...
public class VaryHighlighter {

  private Lexer lexer;
  private Charset charset;

  public VaryHighlighter(VaryConfiguration conf) {
    this.lexer = VaryLexer.create(conf);
  ...
  private void highlightNonComments(Highlightable.HighlightingBuilder highlighting, List<Token> tokens, SourceFileOffsets offsets) {
    for (Token token : tokens) {
      if (VaryTokenType.CADENA.equals(token.getType())) {
  ...
  private boolean isConstant(TokenType type) {
    return VaryTokenType.NUMERO.equals(type)
      || VaryTokenType.CARACTER.equals(type);
  }

  private boolean isKeyword(TokenType type) {
    for (TokenType keywordType : VaryKeyword.values()) {
