package org.sonar.vary.plugin.coverage;
 ...
 * {@inheritDoc}
 */
public class VaryCoverageSensor extends VaryReportSensor {
  private enum CoverageType {
    UT_COVERAGE, IT_COVERAGE, OVERALL_COVERAGE
  }

  public static final String REPORT_PATH_KEY = "sonar.vary.coverage.reportPath";
  public static final String IT_REPORT_PATH_KEY = "sonar.vary.coverage.itReportPath";
  public static final String OVERALL_REPORT_PATH_KEY = "sonar.vary.coverage.overallReportPath";
  ...
  public static final String FORCE_ZERO_COVERAGE_KEY = "sonar.vary.coverage.forceZeroCoverage";
   ...
   * {@inheritDoc}
   */
  public VaryCoverageSensor(Settings settings, FileSystem fs, ProjectReactor reactor) {
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    VaryUtils.LOG.debug("Parsing coverage reports");
    ...
    VaryUtils.LOG.debug("Parsing integration test coverage reports");
    ...
    VaryUtils.LOG.debug("Parsing overall test coverage reports");
    ...
    if (isForceZeroCoverageActivated()) {
      VaryUtils.LOG.debug("Zeroing coverage information for untouched files");
      ...
      for (CoverageParser parser : parsers) {
        try {
          ...
          if (!measuresForReport.isEmpty()) {
            ...
            VaryUtils.LOG.info("Added report '{}' (parsed by: {}) to the coverage data", report, parser);
            break;
          }
        } catch (XMLStreamException e) {
          VaryUtils.LOG.trace("Report {} cannot be parsed by {}", report, parser);
        }
      }

      if (!parsed) {
        VaryUtils.LOG.error("Report {} cannot be parsed", report);
      }
    }
  ...
  private void saveMeasures(Project project,
    ...
    for (Map.Entry<String, CoverageMeasuresBuilder> entry : coverageMeasures.entrySet()) {
      ...
      if (fileExist(context, cxxfile)) {
        VaryUtils.LOG.debug("Saving coverage measures for file '{}'", filePath);
        ...
        VaryUtils.LOG.warn("Cannot find the file '{}', ignoring coverage measures", filePath);
      }
    }
  }
  ...
  private void zeroMeasuresWithoutReports(Project project,
    ...
    for (File file : fs.files(fs.predicates().hasLanguage(VaryLanguage.KEY))) {
      org.sonar.api.resources.File resource = org.sonar.api.resources.File.fromIOFile(file, project);
      if (fileExist(context, resource)) {
        String filePath = VaryUtils.normalizePath(file.getAbsolutePath());
  ...
  private void saveZeroValueForResource(org.sonar.api.resources.File resource,
    ...
    if (ncloc != null && stmts != null
      ...
      VaryUtils.LOG.debug("Zeroing {}coverage measures for file '{}'", coverageKind, filePath);
