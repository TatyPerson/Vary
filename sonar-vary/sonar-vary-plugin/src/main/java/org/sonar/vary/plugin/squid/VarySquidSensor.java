 ...
 * {@inheritDoc}
 */
public final class VarySquidSensor implements Sensor {
   ...
   * {@inheritDoc}
   */
  public VarySquidSensor(ResourcePerspectives resourcePerspectives, Settings conf,
  ...
  public void analyse(Project project, SensorContext context) {
    ...
    this.scanner = VaryAstScanner.create(createConfiguration(this.fs, this.conf),
  ...
  private void highlight() {
    VaryHighlighter highlighter = new VaryHighlighter(createConfiguration(fs, conf));
  ...
  private void save(Collection<SourceCode> squidSourceFiles) {
    ...
    for (SourceCode squidSourceFile : squidSourceFiles) {
      ...
      dependencyAnalyzer.addFile(sonarFile/*, VaryParser.getIncludedFiles(ioFile)*/);
    }

    Measure measure = new Measure(VaryMetrics.SQUID);
  ...
  private void saveMeasures(org.sonar.api.resources.File sonarFile, SourceFile squidFile) {
    context.saveMeasure(sonarFile, CoreMetrics.FILES, squidFile.getDouble(VaryMetric.FILES));
    context.saveMeasure(sonarFile, CoreMetrics.LINES, squidFile.getDouble(VaryMetric.LINES));
    context.saveMeasure(sonarFile, CoreMetrics.NCLOC, squidFile.getDouble(VaryMetric.LINES_OF_CODE));
    context.saveMeasure(sonarFile, CoreMetrics.STATEMENTS, squidFile.getDouble(VaryMetric.STATEMENTS));
    context.saveMeasure(sonarFile, CoreMetrics.FUNCTIONS, squidFile.getDouble(VaryMetric.FUNCTIONS));
    context.saveMeasure(sonarFile, CoreMetrics.COMPLEXITY, squidFile.getDouble(VaryMetric.COMPLEXITY));
    context.saveMeasure(sonarFile, CoreMetrics.COMMENT_LINES, squidFile.getDouble(VaryMetric.COMMENT_LINES));
    context.saveMeasure(sonarFile, CoreMetrics.PUBLIC_API, squidFile.getDouble(VaryMetric.PUBLIC_API));
  }
  ...
  private void saveFunctionsComplexityDistribution(org.sonar.api.resources.File sonarFile, SourceFile squidFile) {
    ...
    for (SourceCode squidFunction : squidFunctionsInFile) {
      complexityDistribution.add(squidFunction.getDouble(VaryMetric.COMPLEXITY));
    }
  ...
  private void saveFilesComplexityDistribution(org.sonar.api.resources.File sonarFile, SourceFile squidFile) {
    ...
    complexityDistribution.add(squidFile.getDouble(VaryMetric.COMPLEXITY));
    context.saveMeasure(sonarFile, complexityDistribution.build().setPersistenceMode(PersistenceMode.MEMORY));
  }
