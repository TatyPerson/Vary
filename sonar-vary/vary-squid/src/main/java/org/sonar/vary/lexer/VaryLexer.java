...
public final class VaryLexer {

  private VaryLexer() {
  }

  public static Lexer create() {
    return create(new VaryConfiguration());
  }
  ...
  public static Lexer create(VaryConfiguration conf) {
    Lexer.Builder builder = Lexer.builder()
        ...
        .withChannel(regexp(VaryTokenType.NUMERO, "[0-9]++\\.[0-9]*+" + opt(EXP) + opt(FLOAT_SUFFIX)))
        .withChannel(regexp(VaryTokenType.NUMERO, "\\.[0-9]++" + opt(EXP) + opt(FLOAT_SUFFIX)))
        .withChannel(regexp(VaryTokenType.NUMERO, "[0-9]++" + EXP + opt(FLOAT_SUFFIX)))

        // C++ Standard, Section 2.14.2 "Integer literals"
        .withChannel(regexp(VaryTokenType.NUMERO, "[1-9][0-9]*+" + opt(INTEGER_SUFFIX))) // Decimal literals
        .withChannel(regexp(VaryTokenType.NUMERO, "0[0-7]++" + opt(INTEGER_SUFFIX))) // Octal Literals
        .withChannel(regexp(VaryTokenType.NUMERO, "0[xX][0-9a-fA-F]++" + opt(INTEGER_SUFFIX))) // Hex Literals
        .withChannel(regexp(VaryTokenType.NUMERO, "0" + opt(INTEGER_SUFFIX))) // Decimal zero
        ...
        .withChannel(new IdentifierAndKeywordChannel(and("[a-zA-Z_]", o2n("\\w")), true, VaryKeyword.values()))

        // C++ Standard, Section 2.13 "Operators and punctuators"
        .withChannel(new PunctuatorChannel(VaryPunctuator.values()))
